...
export class AttributeComponent extends Component {
	...
	onInit() {
		...
		const attributes = ATTRIBUTES.filter(x => node.hasAttribute(`${x}_`));
		const values = attributes.map(x => Component.getExpression(node.getAttribute(`${x}_`)));
		this.state$.subscribe(state => {
			attributes.forEach((attribute, i) => {
				node.setAttribute(attribute, values[i](state));
			});
		});
		/*
		const xattr = this.selector.replace(/\[|\]/g, '');
		const attr = xattr.substr(1, xattr.length - 1);
		const expression = node.getAttribute(xattr);
		const getValue = Component.getExpression(expression);
		this.state$.subscribe(state => {
			node.setAttribute(attr, getValue(state));
		});
		*/
	}

	static meta = {
		selector: `[${ATTRIBUTES.join('_],[')}_]`,
	};
...
open		<details>, <dialog>		Indicates whether the the contents are currently visible (in the case of a <details> element) or whether the dialog is active and can be interacted with (in the case of a <dialog> element).
