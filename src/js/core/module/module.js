...
export class Module {
		...
		return new Promise((resolve, reject) => {
			if (Array.isArray(factory)) {
				...
				if (store.factories.has(name)) {
					...
					const src = factory[1];
					const key = Object.keys(lazyModules).find(key => key.indexOf(src) !== -1);
					if (key) {
		...
		return Promise.all(metas.map(meta => {
			return this.getFactory(meta).then(factory => {
				if (typeof factory === 'object') {
					...
					const instance = new factory.prototype.constructor(node, data, subject, this, originalNode);
					return instance;
				}
			});
		}));
	...
	register(node = document) {
		...
		return Promise.all(observingNodes.map(node => {
			...
			return this.init(metas, node, data, subject, originalNode);
		})).then(items => {
	...
	observe$(node = document) {
		...
		const initialize = (node) => {
			if (results.has(node)) {
				...
				const metas = results.get(node);
				const subject = new Subject();
				this.init(metas, node, data, subject, originalNode).then(items => {
...
export function useModule(metas) {
	metas = parseModule(metas);
	return new Module(metas);
}

export function parseModule(module) {
	const factories = [];
	const pipes = {};
	if (module.imports) {
		module.imports.forEach(x => {
			const m = parseModule(x);
	...
	if (module.factories) {
		factories.push(...module.factories);
	}
	if (module.pipes) {
		module.pipes.forEach(x => {
			pipes[x.meta.name] = x;
		});
	}
	return { factories, pipes };
}
